...
namespace Lab5
{
    internal class EmpTable
    {
        ...
        //Create new employe
        public void addItem(Employe emp)
        {
           SqlConnection conn = connection();
            string query = @"insert into Emp_Detail(name, designation, department, salary) values('" + emp.getName() + "', '" + emp.getDesignation() + "', '" + emp.getDepartment() + "'," + emp.getSalary() + ")";
            SqlCommand cmd = new SqlCommand(query, conn);
            try
            {
                conn.Open();
                cmd.ExecuteNonQuery();
                Console.WriteLine("Employe :\nName : {0}\tDesignation : {1}\tDepartement : {2}\tSalary : {3} Rs", emp.getName(), emp.getDesignation(), emp.getDepartment(), emp.getSalary());
            }
            catch (Exception ex)
            {
                Console.WriteLine("Message : " + ex.Message);
            ...
            while (bol)
            {
                ...
                switch (ch)
                {
                    case 1:
                        ...
                        emp = new Employe(name, designation, department, salary);
                        empTable.addItem(emp); break;
                    case 2:
